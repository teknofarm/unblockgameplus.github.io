...
const searchResultCache = new SearchCache(1000); // Cache last 1000 unique queries
...
async function buildIndex() {
  ...
  try {
    if (!fs.existsSync(keywDir)) {
      console.warn(`[Index] Warning: Index directory not found at ${keywDir}`);
      return;
    }
    ...
    for (const f1 of level1Folders) {
      ...
      for (const item of level2Items) {
        ...
        for (const line of lines) {
          ...
          const entry = { code, f1, f2, keyword };
    ...
    totalArticles = count;
    lastBuild = Date.now();
    searchResultCache.clear(); // Clear search cache when index is rebuilt
  ...
  for (const kw of relatedKeywords) {
    ...
    for (const entry of searchIndex) {
      ...
      if (qWords.every(w => entry.keyword.toLowerCase().includes(w))) {
        // Clone entry and add matchedKeyword for better anchor text matching
